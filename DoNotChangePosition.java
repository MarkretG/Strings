    ...
    static void reverse(char[] s)
    {
        ...
        while(i<j)
        {
            if((s[i]>=32&&s[i]<=47) || (s[i]>=58&&s[i]<=64) ||(s[i]>=91&&s[i]<=96)|| (s[i]>=123&&s[i]<=126))
                ++i;
            if((s[j]>=32&&s[j]<=47) || (s[j]>=58&&s[j]<=64) || (s[j]>=91&&s[j]<=96)||(s[j]>=123&&s[j]<=126))
                --j;
            if((i<j)&&((s[i]>=97&&s[i]<=122)||(s[i]>=48&&s[i]<=57)||(s[i]>=65&&s[i]<=97))&&((s[j]>=97&&s[j]<=122)||(s[j]>=48&&s[j]<=57)||(s[j]>=65&&s[j]<=97)))
            {
                t=s[i];
                s[i]=s[j];
                s[j]=t;
